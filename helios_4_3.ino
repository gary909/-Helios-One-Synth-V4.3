...
//byte cutoff_freq = cutVal;  // Set the cuttoff frequency to that of the pot 
  ...
  int LFOrateVal = mozziAnalogRead(LFOratePot);    // read the value from the attack pot
  LFOrateVal = LFOratePotMap(LFOrateVal);

  int LFOresVal = mozziAnalogRead(LFOresPot);    // read the value from the attack pot
  LFOresVal = LFOresPotMap(LFOresVal);
...
//*****************************ADD LFO**********************************************************************************
  ...
  int LFOrate_freq = 2200 + kFilterMod.next()*12; // 
...
//  return (int) (envelope.next() * oscil1.next())>>8; // original here, but now we need to pass filter as well
  char asig = (envelope.next() * oscil1.next())>>10; // changed down from 10 to 9 because was louder
//  return (lpf.next(asig) * svf.next());
//  return (svf.next(asig));
//  return (lpf.next(asig));

    char asigLPF = (svf.next(asig));
    return (lpf.next(asigLPF));
